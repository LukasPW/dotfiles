// LEGACY - kept for reference only. Not imported by shell.qml.
//
// This is the original Hyprland-only workspace switcher. It has been
// superseded by Modules/Workspaces.qml, which keeps this exact rendering
// and click behaviour but sources its state through a per-compositor
// service (Modules/Wm/HyprlandService.qml / Modules/Wm/NiriService.qml)
// so the same bar works under both Hyprland and niri.
//
// The Hyprland code path in HyprlandService.qml is a straight lift of the
// logic below (same `Hyprland.focusedWorkspace` / `Hyprland.workspaces`
// reads, same `hl.dsp.focus` dispatch string), so Hyprland behaviour is
// unchanged. Refer here if you need to see the pre-abstraction version.
