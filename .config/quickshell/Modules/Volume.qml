...
Text {
    ...
    text: {
        const speaker = volumeIcon(volume, muted) + " " + Math.round(volume * 100) + "%"
        const mic = micMuted ? ("  <font color=\"" + Theme.error + "\">\uf131</font>") : ""  // fa-microphone-slash
        return speaker + mic
    }
    textFormat: Text.StyledText
    color: muted ? Theme.error : Theme.secondary
    font {
    ...
    // Keeps the sink/source bound so their properties actually update -
